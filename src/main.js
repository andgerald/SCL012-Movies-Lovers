...
document.getElementById("buttonPlayDcComics").addEventListener("click", function (){
  ...
  for (let i = 0; i < DCCOMICS.length; i += 1) {
    document.getElementById('results0').innerHTML += `<div class ="trivia"><ul class='lista'><li>PREGUNTA ${DCCOMICS[i].numberQuestion}</li><br><li> ${DCCOMICS[i].question}</li><br><li>${DCCOMICS[i].answers.A}</li><li> ${DCCOMICS[i].answers.B}</li><li>${DCCOMICS[i].answers.C}</li></ul></div><input type='text' name="answer${i}" id="answer${i}" class="texts" value="hola">`;
  }
}); 
